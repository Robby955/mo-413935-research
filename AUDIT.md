...
| A fixed number of occupancy moments or mixed-cycle layers universally decides whether a relative-gauge fiber is empty. | flawed | `RESEARCH_CONTINUATION.md`, Theorem 42A | The even and odd binomial-multiplicity multisets have identical moments through degree \(D\), but only the even multiset contains zero. On \(2^{N-1}\) abstract fibers, moments through degree \(N-1\) can therefore fail. These abstract multisets are a generic obstruction, not claimed realizable signing shells. |
