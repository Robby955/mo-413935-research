...
insufficient.  Retaining the nonlinear arcsine term gives a trace-four
stability bound and the new finite consequence $F(21)\ge32$.  We derive an
exact multivertex weighted Bellman identity, a weighted entropy upper bound,
and density-one control of Bellman-optimal predecessors.  A complete
order-nine catalogue shows that even the energy histogram plus the two-point
overlap law of exact maximizers does not determine extension.  The sharp
remaining target is a
...
$\|A_n^2-(n-1)I\|_F=o(n^2)$.  The universal improvement in (26) is only
...
$\sqrt{nk(n+k)}$ block cost.  A sufficient replacement is a constant-matching
profile law: find $B,D$ and $\varepsilon>0$ such that
\[
...
standard blocking argument turns into convergence; (31) is a target, not a
proved estimate.
...
$\delta_n\le\frac12g(N)\sqrt N$.  Sparse exceptional orders remain
uncontrolled, so this does not prove convergence.
...
retain enough energy layers to control the weighted radius without paying a
leading-order entropy loss.
