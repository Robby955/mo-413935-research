...
## Exact values through order 15 by lifting

This is a second, cheaper route to orders 13 and 14, and the only route to
order 15. It supersedes the pruned order-13 scan described above for the
purpose of establishing the table; that scan is retained as an independent
check, not deleted.

Two facts do the work. Every energy is a sum of `choose(n,2)` terms equal to
`+1` or `-1`, so

```text
F(n) = choose(n,2)  (mod 2),
```

and deleting a vertex cannot raise the maximum, so a signing of order `n+1`
with `M <= T` restricts to one of order `n` with `M <= T`. Writing `B_n(T)`
for a set of class representatives of order `n` with `M <= T`, every element
of `B_{n+1}(T)` is a one-vertex extension of an element of `B_n(T)`, and the
threshold at each order may be lowered to the nearest integer of the right
parity.

### Scripts

```text
verify_F_exhaustive.py   unpruned enumeration of F(n) for n <= 12
collect_min12.py         the same, collecting every record with M <= THRESH
lift.py                  one-vertex lift of a small record set, dense
expand.py                extension and renormalisation of a single record
expand_big.py            shared tables for the batched extension
expand_prune.py          batched extension with the extreme-energy prune
hashfilter.py            splits a canonical stream into disjoint passes
sample_T.py              sharded geng pass collecting records below a threshold
tower25.sh               the three-level tower that settles order 15
full_final_check.py      independent recheck of the tower's last level
class_count.py           switching classes, as opposed to rooted records
mval.py                  recomputes M of a record, for witness checking
minmax_search.py         exact M by full enumeration, and Paley construction
```

### Reproducing the table

```bash
python3 verification/verify_F_exhaustive.py 10 11 12
...
```text
n=10  graphs_on_9_vertices=274668  F(10)=13
n=11  graphs_on_10_vertices=12005168  F(11)=17
n=12  graphs_on_11_vertices=1018997864  F(12)=18
min M over all one-vertex extensions = 24
order12_minimisers.g6: 2 rooted records, 1 switching classes
```

The three graph counts equal the published numbers of non-isomorphic graphs on
9, 10 and 11 vertices. Orders 10 and 11 take seconds and minutes; order 12
takes about 20 minutes. The lift returning 24 is what rules out `F(13) <= 18`,
after which parity gives `F(13) = 20` and then `F(14) = 21` with no further
computation.
...
Needs nauty, numpy, about 15 GB of scratch and 15 to 20 hours on 16 cores.
`PASSES` trades disk against wall time; the default 8 keeps the sorts inside a
15 GB budget. Level sizes, printed as it runs:

```text
|T12(M<=24)| = 82502142
|T13(M<=24)| = 282202131
|T14(M<=25)| = 1313164
T15(M<=25) EMPTY  =>  F(15) = 27
```

The empty final level says no order-15 signing has maximum at most 25. Parity
lifts that to 27, and extending the order-14 Paley matrix over all `2^14` sign
rows attains 27 exactly, with maxima `27,29,31,33,35` occurring `28, 1820,
4732, 1456, 156` times.

### Rechecking the last level on its own

```bash
python3 verification/full_final_check.py T14_thresh25.g6 --res 0 --mod 1
```

This re-lifts every order-14 record and reports any extension with `M <= 25`.
It shares no code with the tower. Shard it with `--res`/`--mod` to run in
parallel; twelve shards finish in a few minutes. Expected output per shard is
`hits<=25=0`.

The prune inside it is a relaxation: a sign row is rejected only when it
already violates the threshold on one of the `K` most extreme energies, which
is a necessary condition, so it can keep too many candidates but never discard
a real one. Survivors are then checked over all `2^14` sign vectors. Raising
the threshold to 27 recovers the Paley witness, which is the control that its
null result at 25 is not vacuous.

### Trust boundary

The order-12 seed trusts nauty's isomorph-free generation, as the earlier
scans do. Everything above order 12 is arithmetic on that seed: no solver, no
timeout inference, and no pruning filter that can discard a candidate. The
`T14_thresh25.g6` level is 19.7 MB and is not stored in the repository; it is
regenerated by `tower25.sh`. The small levels that are stored are
`order12_minimisers.g6`, `T13_thresh21.g6`, `T14_thresh21.g6` and
`T14_thresh23.g6`.
